...
public class Tambah_simpul {
    ...
    public void dobelSimpul(int nodes0, int nodes1, int index_koordinat_json,
                            Context context, String[][] graph,
        ...
        //Hitung simpul yang aslinya dulu (5-4), bukan yang dobelnya (4-5)
        //================================================================
        ...
        for(int k = 0; k < 100; k++) {
            ...
            if (graph[nodes0][k] != null){

                String simpulAwal = graph[nodes0][k]; // [5][0] = 4->721.666

                //4->721.666
                String [] explode = simpulAwal.split("->");

                simpul_tujuan = explode[0]; //4

                //jika 4 == 4 (node1)
                if (simpul_tujuan.trim().equals(String.valueOf(nodes1).trim())){
                    //index kolom example graph[baris][kolom]
                    index_kolom_graph = String.valueOf(k);
                }
            }else break;
        }//for
        ...
        //replace array graph [5][0] = 6->88.6
        graph[baris][kolom] = (max_simpul_db+1)+"->"+ct.bobot;

        int start_loop = 0;
        //buat dan simpan (new record) json koordinat yang bary ke DB
        ...
        graph[(max_simpul_db+1)][0] = nodes1 + "->" + ct.bobot; //definisikan [0] karena index baru di graph [][]
        ...
        for (int k = 0; k < 100; k++){
            ...
            if(graph[t_nodes0][k] != null){

                //== dapatkan simpul tujuan, contoh : 5 ->9585.340
                String simpulAwal = graph[t_nodes0][k];
                String [] explode1 = simpulAwal.split("->");

                nodes_inside_kolom = explode1[0];

                if(nodes_inside_kolom.trim().equals(String.valueOf(t_nodes1))){
                    index_kolom_graph1 = String.valueOf(k);
                }
        ...
        ct.Count_Bobot_Tambah_Simpul(0, index_dobel_koordinat_json, jArrCoordinates1); //0, koordinate awal ke tengah, JSONArray Coordinates
    ...
    public void singleSimpul(int nodes0, int nodes1, int index_koordinat_json,
        ...
        //cari index kolomnya nodes_akhir1 (4) dari graph [baris][kolo]
        for (int k = 0; k < 100; k++){
            ...
            if (graph[nodes0][k] !=  null){
                ...
                if (value_node_array.trim().equals(String.valueOf(nodes1).trim())){
                    ...
                    //index kolom; contoh graph [baris][kolom]
                    index_kolom_graph = String.valueOf(k);
            ...
            //ambil koordinat nya dari simpul 3-6
            ...
            int limit1      = (jArrCoordinates.length() -1); // - 1 karena array mulai dari 0
            ct.Count_Bobot_Tambah_Simpul(index_koordinat_json, limit1, jArrCoordinates);
    ...
    private void createAndSave_NewJsonCoordinate(int mulai, int limit, JSONArray jArrCoordinates,
                                                 int new_id, int field_simpul_awal, int field_simpul_akhir,
                                                 double new_bobot, SQLiteDatabase dbInsert, SQLiteDatabase dbRead) throws JSONException {
        ...
        for(int ne = mulai; ne <= limit; ne++){
            ...
            JSONArray latlong   = jArrCoordinates.getJSONArray(ne);
            double new_lat      = latlong.getDouble(0);
            double new_long     = latlong.getDouble(1);

            JSONArray new_list_coordinates = new JSONArray();
            new_list_coordinates.put(new_lat);
            new_list_coordinates.put(new_long);
